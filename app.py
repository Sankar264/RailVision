...
# --- 7. MAIN DASHBOARD WORKSPACE ---
    ...
    # --- OPERATIONS & DISRUPTION SIMULATOR VIEW ---
        ...
        st.markdown("<h2 style='color:#f8fafc;'>📈 Operations Control & Disruption Monitor</h2>", unsafe_allow_html=True)
        st.caption("Read-only view of network resilience under simulated track blockages or emergency maintenance.")
        st.markdown("""
        ...
        with sim_col1:
            st.markdown("""
                <div style="background: #131c2e; border: 1px solid #1e293b; padding: 20px; border-radius: 12px; margin-bottom: 20px;">
                    <h3 style="color: #f8fafc; margin-top: 0;">⚡ Disruption Status Monitor</h3>
                    <p style="color: #94a3b8; font-size: 0.9rem;">Live read-only status of emergency line blocks, signal failures, or severe weather incidents and their real-time cascade delay impact.</p>
                </div>
            """, unsafe_allow_html=True)

            current_sim_state = st.session_state.simulated_blockage
            if current_sim_state:
                st.error("⚠️ ACTIVE INCIDENT: Major Track Blockage in progress (+45m cascade delay applied across network).")
            else:
                st.success("🟢 Network Operating normally under optimal scheduling vectors.")
            st.caption("This portal is read-only. Incident triggers are managed by the automated monitoring system.")
    ...
    # --- PASSENGER FEEDBACK & REVIEWS VIEW ---
        ...
        is_read_only_role = st.session_state.user_role in ("Operations", "Admin")
        if is_read_only_role:
            st.caption("Read-only view of ratings submitted by passengers across Indian Railways.")
        else:
        ...
        if is_read_only_role:
            st.markdown("""
                <div style="background: #131c2e; border: 1px solid #1e293b; padding: 20px; border-radius: 12px; margin-bottom: 20px;">
                    ...
                    <p style="color: #94a3b8; font-size: 0.9rem;">Real-time read-only stream of all passenger reviews stored securely in SQLite. Feedback submission is available to passengers only.</p>
                </div>
            """, unsafe_allow_html=True)
            feedback_df = get_all_feedback()
            if not feedback_df.empty:
                st.dataframe(feedback_df, use_container_width=True, height=500)
            else:
